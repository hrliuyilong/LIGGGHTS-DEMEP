...
//addbylyl
      else if (strcmp(arg[iarg + 1], "CoeffCGMethod") == 0)
      {
          if (narg < iarg + 2)
              error->fix_error(FLERR, this, "not enough arguments for 'CoeffCGMethod'");
          iarg += 2;
          useCGMethod_ = true;
          CoeffCGMethod_ = atof(arg[iarg]);
          fprintf(screen, "DEM will consider Coarse-grained Method with CoeffCGMethod = %f\n",
              CoeffCGMethod_);
      }
//
...
//kcg mod
      omega[i][0] += pow(CoeffCGMethod_, 2) * dtirotate * torque[i][0];
      omega[i][1] += pow(CoeffCGMethod_, 2) * dtirotate * torque[i][1];
      omega[i][2] += pow(CoeffCGMethod_, 2) * dtirotate * torque[i][2];
//
    }
  }
