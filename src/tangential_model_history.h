...
//
        CoeffCG_TM(NULL),    //kcg
        coeffStaticFrict(NULL), //the static friction
        referenceVelocity(NULL),    //the Vw for friction relation
        Yeff(NULL),
//
        heating(false),
        heating_track(false),
...
//
        registry.registerProperty("CoeffCG_TM", &MODEL_PARAMS::createCoeffCGMethod, "tangential_model history");
        registry.connect("CoeffCG_TM", CoeffCG_TM, "tangential_model history");

        registry.registerProperty("coeffStaticFrict", &MODEL_PARAMS::createCoeffStaticFrict, "tangential_model history");
        registry.connect("coeffStaticFrict", coeffStaticFrict, "tangential_model history");

        registry.registerProperty("referenceVelocity", &MODEL_PARAMS::createCoeffRefVelocity, "tangential_model history");
        registry.connect("referenceVelocity", referenceVelocity, "tangential_model history");

        registry.registerProperty("Yeff", &MODEL_PARAMS::createYeff, "tangential_model history");
        registry.connect("Yeff", Yeff, "tangential_model history");
...
//static friction
        ...
        double xmu = coeffFrict[sidata.itype][sidata.jtype];

//static friction
        double xmu_static = coeffStaticFrict[sidata.itype][sidata.jtype];
        const double Uw_mod = referenceVelocity[sidata.itype][sidata.jtype];    //ref velocity for frition relation
//cgmethod

        const double kcg = CoeffCG_TM[sidata.itype][sidata.jtype];
        ...
        double Ureltmag_vtr = sqrt(sidata.vtr1 * sidata.vtr1 + sidata.vtr2 * sidata.vtr2 + sidata.vtr3 * sidata.vtr3);
        
        double Ureltmag_ = Ureltmag_vtr;

        double rhos_ = sidata.mi / (4.0 / 3.0 * M_PI * sidata.radi * sidata.radi * sidata.radi);

        double Uw_ = Uw_mod * sqrt(sidata.kn / pow(kcg, 2) / rhos_ / (2 * reff / kcg) / (1e6));
 
        //mod in dynamic friction
        double xmu_eff = xmu_static;
        double xmu_eff_0 = xmu_static;

        xmu_eff = xmu / kcg + (xmu_static - xmu / kcg) * pow(2.718281828459046, -(Ureltmag_ / Uw_));
        xmu_eff_0 = xmu + (xmu_static - xmu) * pow(2.718281828459046, -(Ureltmag_ / Uw_));
        ...
        if (xmu_eff_0 > xmu_static)
        {
            xmu_eff_0 = xmu_static;
        }
        //end
        ...
        const double Ft_friction =  xmu_static * fabs(sidata.Fn);   //static friction
 
        // energy loss from sliding or damping
        if (Ft_shear > Ft_friction) {    //dynamic friction case

          if (shrmag != 0.0) {
            const double ratio = xmu_eff / xmu_static * Ft_friction / Ft_shear;
            ...
            if (update_history)
            {
                shear[0] = -Ft1 / kt / (xmu_eff / xmu_static);
                shear[1] = -Ft2 / kt / (xmu_eff / xmu_static);
                shear[2] = -Ft3 / kt / (xmu_eff / xmu_static);
        ...
        // forces & torques
        double tor1 = (eny * Ft3 - enz * Ft2) * kcg;
        double tor2 = (enz * Ft1 - enx * Ft3) * kcg;
        double tor3 = (enx * Ft2 - eny * Ft1) * kcg;
        
        if (Ft_shear > Ft_friction) {
            tor1 = (eny * Ft3 - enz * Ft2) * kcg / (xmu_eff / xmu_eff_0);
            tor2 = (enz * Ft1 - enx * Ft3) * kcg / (xmu_eff / xmu_eff_0);
            tor3 = (enx * Ft2 - eny * Ft1) * kcg / (xmu_eff / xmu_eff_0);
        }
