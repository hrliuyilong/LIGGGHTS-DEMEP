...
namespace ContactModels {
  ...
  public:
	...
	//no capilar force addbylyl
      //const double Fcapilary = - M_PI*surfaceTension*sqrt(radi*radj)*(exp(Bparam)+Cparam);
      const double Fcapilary = - 0.0 * M_PI*surfaceTension*sqrt(radi*radj)*(exp(Bparam)+Cparam);
...
bridge_breaks = false;
	...
	//no capilar force addbylyl
          ...
          const double Fcapilary = - 0.0 * M_PI*surfaceTension*sqrt(radi*radj)*(exp(Aparam*dist/R2+Bparam)+Cparam);
