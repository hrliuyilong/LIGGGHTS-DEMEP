...
//
    ...
    {
//
        vn_init_offset_ = cmb->get_history_offset("vn_init_history");
        if (vn_init_offset_ == -1)
        {
            vn_init_offset_ = hsetup->add_history_value("vn_init_history", "0");
            if (cmb->is_wall())
                hsetup->add_history_value("vn_init_history_wall", "0");
            cmb->add_history_offset("vn_init_history", vn_init_offset_);
        }
//
        if (elasticpotflag_)
        {
...
//
      registry.registerProperty("CoeffRest", &MODEL_PARAMS::createCoeffRest, "model hertz");
      registry.registerProperty("CoeffCGMethod", &MODEL_PARAMS::createCoeffCGMethod, "model hertz");
//
...
//
      registry.connect("CoeffRest", CoeffRest, "model hertz");
      registry.connect("CoeffCGMethod", CoeffCGMethod, "model hertz");
//
...
//addbylyl cgmethod 
//kcg mod
      const double kcg = CoeffCGMethod[sidata.itype][sidata.jtype];
      const double radi = pow(kcg, -1) * sidata.radi;
      const double radj = pow(kcg, -1) * sidata.radj;
//end
...
//
      //cgmethod
      const double meff = pow(kcg, -3) * sidata.meff;
//
...
//
      ...
      double betaeff_ = betaeff[itype][jtype];
      double betaefft_ = betaeff[itype][jtype]; //only mod in normal dir
      const double Sn = 2. * Yeff[itype][jtype] * sqrtval;
      const double St = 8. * Geff[itype][jtype] * sqrtval;

//cgmethod

      double kn = 4. / 3. * Yeff[itype][jtype] * sqrtval * pow(kcg, 1.5);
      double kt = St * pow(kcg, 1.5);
      const double sqrtFiveOverSix = 0.91287092917527685576161630466800355658790782499663875;
      const double gamman = -2. * sqrtFiveOverSix * betaeff_ * sqrt(Sn * meff) * 
          pow(kcg, 2.75);
      const double gammat = tangential_damping ? -2. * sqrtFiveOverSix * betaefft_ * sqrt(St * meff) * 
          pow(kcg, 2.75) : 0.0;
//
